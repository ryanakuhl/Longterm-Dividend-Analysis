...
import xlrd, requests, json, time, warnings
...
def set_the_att(returned_dict):
    for a in returned_dict:
        try:
            ...
            print('line 56', e, returned_dict)

def advancing_research(allstocks):
    if allstocks.latestPrice > (allstocks.week52High * .9):
        pass
    elif allstocks.latestPrice < (allstocks.week52Low * 1.4):
        pass
...
for a in list(set(sh)):
    try:
            ...
            if a.row(rx)[0] not in block_dups:
                ...
                if allstocks.funt_init_analyze():
                    ...
                    if hasattr(allstocks, 'week52Low') and type(allstocks.week52Low) is int or type(allstocks.week52Low) is float:
                        if advancing_research(allstocks):
        ...
        print('line 97 ', a.row(rx)[0], e)
    time.sleep(5)

def used_to_be_free():
    """not available to free acounts anymore"""
